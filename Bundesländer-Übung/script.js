...
function addComment(index) {
    let input = document.getElementById(`input${index}`); // input als variable definiert damit wir darauf zugreifen können ${}so wird ensprechende stelle genommen input 2,3 usw.
    bundeslaender[index]['comments'].push(input.value); // array bundeslaender [index]entsprechende stelle ['auf id comments im json array'].push() speichert im local storage die variable input.value die wir vorher definiert haben.
    render();
    input.value = '';
}